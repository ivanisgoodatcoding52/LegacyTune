...
+ (LTPlaylistStore *)sharedStore {
	if (_sharedStore == nil) _sharedStore = [[LTPlaylistStore alloc] init];
	return _sharedStore;
}
...
- (NSArray *)allPlaylists {
	...
	for (NSDictionary *row in rows) [playlists addObject:[LTPlaylist playlistWithRow:row]];
	return playlists;
}
...
- (LTPlaylist *)createPlaylistWithName:(NSString *)name {
	...
	NSArray *rows = [db executeQuery:@"SELECT * FROM playlists WHERE id = ?"
		withArguments:[NSArray arrayWithObject:[NSNumber numberWithLongLong:[db lastInsertRowId]]]];
...
- (NSArray *)songsInPlaylist:(LTPlaylist *)playlist {
	NSString *sql = @"SELECT songs.* FROM songs INNER JOIN playlist_items ON playlist_items.song_id = songs.id WHERE playlist_items.playlist_id = ? ORDER BY playlist_items.sort_order ASC";
	NSArray *rows = [[LTDatabase sharedDatabase] executeQuery:sql withArguments:[NSArray arrayWithObject:[NSNumber numberWithInteger:playlist.playlistId]]];
...
- (void)addSong:(LTSong *)song toPlaylist:(LTPlaylist *)playlist {
	...
	[db executeUpdate:@"INSERT INTO playlist_items (playlist_id, song_id, sort_order) VALUES (?, ?, ?)"
		withArguments:[NSArray arrayWithObjects:
			...
			[NSNumber numberWithInteger:nextSort], nil]];
}

- (void)removeSongAtIndex:(NSUInteger)index fromPlaylist:(LTPlaylist *)playlist {
	NSArray *songs = [self songsInPlaylist:playlist];
	...
	[[LTDatabase sharedDatabase] executeUpdate:@"DELETE FROM playlist_items WHERE playlist_id = ? AND song_id = ?"
		withArguments:[NSArray arrayWithObjects:
			[NSNumber numberWithInteger:playlist.playlistId],
			[NSNumber numberWithInteger:song.songId], nil]];
}
