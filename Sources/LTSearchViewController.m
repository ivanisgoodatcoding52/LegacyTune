...
- (void)loadView {
	...
	_emptyStateLabel.textAlignment = LTTextAlignmentCenter;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
	...
	_debounceTimer = [NSTimer scheduledTimerWithTimeInterval:kLTSearchDebounceInterval target:self selector:@selector(searchTextChanged) userInfo:nil repeats:NO];
}
...
- (void)searchTextChanged {
	...
	_searchGeneration++;
	NSDictionary *job = [NSDictionary dictionaryWithObjectsAndKeys:queryText, @"query", [NSNumber numberWithUnsignedInteger:_searchGeneration], @"generation", nil];
...
- (void)runSearchJob:(NSDictionary *)job {
	...
	NSString *sql = @"SELECT * FROM songs WHERE title LIKE ? OR artist LIKE ? OR album LIKE ? OR genre LIKE ? ORDER BY title COLLATE NOCASE ASC LIMIT ?";
	NSArray *args = [NSArray arrayWithObjects:likePattern, likePattern, likePattern, likePattern, [NSNumber numberWithUnsignedInteger:kLTSearchResultLimit], nil];
	...
	NSDictionary *payload = [NSDictionary dictionaryWithObjectsAndKeys:songs, @"results", generation, @"generation", nil];
...
- (void)deliverResults:(NSDictionary *)payload {
	...
	if ([resultGeneration unsignedIntegerValue] != _searchGeneration) return;
	[_results release];
...
- (void)updateEmptyState {
	...
	_emptyStateLabel.text = (_results != nil) ? @"No matches." : @"Search your library — type at least 2 characters.";
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[[LTPlaybackController sharedController] playSongs:_results startingAtIndex:indexPath.row];
}
